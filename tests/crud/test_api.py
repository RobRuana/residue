...
# from residue.crud.orm import crudable, regex_validation, text_length_validation
# from residue.types import JSON, UUID
...
#         assert \
#             sorted(expected, key=lambda m: m.get('id', m.get('_model'))) == \
#             sorted(results, key=lambda m: m.get('id', m.get('_model')))
...
#         }
#         self.assert_read_result(
#             result,
#             query_from(db.turner_account, 'username'),
#             {'username': True})
#         self.assert_read_result(
#             result,
#             [query_from(db.turner_account, 'username')],
...
#             'username')
#         self.assert_read_result(
#             result, [query_from(db.turner_account, 'username')], 'username')

#     def test_handle_read_with_data_spec_requesting_unreadable_attribute(
#             self, db):
#         result = {
#             '_model': 'Account',
#             'id': db.turner_account['id']
...
#         self.assert_read_result(
#             [db.turner_account, db.hooch_account],
#             [query_from(db.turner_account), query_from(db.hooch_account)])

#     def test_read_with_multiple_queries_and_one_data_spec(self, db):
#         expected = self.extract(
#             [db.turner_account, db.hooch_account], '_model', 'id', 'username')
#         self.assert_read_result(
#             expected,
#             [query_from(db.turner_account), query_from(db.hooch_account)],
#             {'username': True})
...
#         expected = \
#             self.extract(db.turner_account, '_model', 'id', 'username') + \
#             self.extract(db.pirate, '_model', 'id', 'user_id')
#         self.assert_read_result(
#             expected, query, [['username'], ['user_id']])
#         self.assert_read_result(
#             expected, query, [{'username': True}, ['user_id']])
#         self.assert_read_result(
#             expected, query, [['username'], {'user_id': True}])
#         self.assert_read_result(
#             expected, query, [{'username': True}, {'user_id': True}])
...
#         results = Session.crud.read(
#             query_from(db.turner), {'does_not_exist': True})
...
#         Session.crud.update(
#             {'_model': 'Account', 'username': 'turner_account'},
#             {'password': 'barbaz', 'user': {'name': 'Turner the Awesome'}})
#         with Session() as session:
#             assert 'barbaz' == session.account('turner_account').password
#             assert 'Turner the Awesome' == \
#                 session.account('turner_account').user.name

#     def test_handle_bad_relation_type(self, db):
#         pytest.raises(CrudException, Session.crud.update,
#                       query_from(db.turner),
#                       {'employees': 'not a dict or sequence of dicts'})

#     def test_create_foreign_relation_with_one_spec(self):
#         Session.crud.update({'_model': 'Account'},
#                             {'user': {'name': 'New User'}})
#         with Session() as session:
...
#     def test_unset_nullable_foreign_relation_from_parent_with_empty_list(
#             self, db):
...
#         pytest.raises(Exception, Session.crud.update, query_from(db.turner),
#                       {'does_not_exist': 'foo'})

#     def test_update_nonupdatable_attribute(self, db):
#         pytest.raises(Exception, Session.crud.update,
#                       query_from(db.turner_account), {'username': 'foo'})


# class TestCrudDelete(object):
#     def test_delete_cascades_to_tags(self):
#         pytest.skip(
#             'SQLite is not compiled with foreign key support on Jenkins; '
#             'this test works on my machine but not on Jenkins')
...
#         pytest.raises(CrudException, Session.crud.delete,
#                       {'_model': 'Account'})
#         pytest.raises(CrudException, Session.crud.delete,
#                       {'_model': 'Tag', 'field': 'name', 'value': 'Male'})
...
#         })
#         with Session() as session:
#             assert {'New', 'Ninja', 'Male'} == \
#                 {tag.name for tag in session.user('Turner').tags}
...
#         pytest.raises(CrudException, Session.crud.create,
#                       {'_model': 'User', 'name': 'Turner'})
...
#         assert results == [{'_model': 'Human', '_label': 'Human'},
#                            {'_model': 'Proxy', '_label': 'Proxy'}]
#         results = normalize_query(['Proxy', 'Human'])
#         assert results == [{'_model': 'Proxy', '_label': 'Proxy'},
#                            {'_model': 'Human', '_label': 'Human'}]
...
#             '_model': 'Human',
#             'or': [
#                 {'_model': 'Human', 'field': 'nickname', 'value': 'Johnny'},
#                 {'_model': 'Human', 'field': 'nickname', 'value': 'Winny'}
#             ]}, {'_model': 'Proxy'}])
#         assert results == [{
#             '_model': 'Human',
#             'or': [
...
#             '_model': 'Human',
#             'or': [
#                 {'field': 'nickname', 'value': 'Johnny'},
#                 {'field': 'nickname', 'value': 'Winny'}
#             ]}, {'_model': 'Proxy'}])
#         assert results == [{
#             '_model': 'Human',
#             'or': [
...
#         pytest.raises(
#             ValueError, normalize_query,
#             [{'or': [[], []]}, {'_model': 'Proxy', '_label': 'Proxy'}])
...
#         pytest.raises(
#             ValueError, normalize_query,
#             [{'_model': 'Proxy'}, {'field': 'nickname', 'value': 'Johnny'}])
...
#         pytest.raises(
#             ValueError, normalize_query,
#             [{'_model': 'Human', 'and': [[], []]}, {'_model': 'Proxy'}])

#     def test_fails_or_clause_with_model_list_of_lists(self):
#         pytest.raises(
#             ValueError, normalize_query,
#             [{'_model': 'Human', 'or': [[], []]}, {'_model': 'Proxy'}])
