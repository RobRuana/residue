...
from residue import normalize_query, check_constraint_naming_convention, crudable, declarative_base, \
    regex_validation, text_length_validation, CrudException, JSON, SessionManager, UUID
from tests import patch_session
...
class CrudableClass(CrudableMixin, Base):
    ...
    def string_and_int_hybrid_property(cls):
        ...
        ], else_=(cls.string_model_attr + ' ' + cls.int_model_attr))  # noqa: E711
...
def init_db(request):
    ...
    db.turner_account = create(
        'Account', username='turner_account', password='password', user_id=db.turner['id'], boss_id=db.boss['id'])
...
def db(request, init_db):
    shutil.copy('/tmp/residue.db', '/tmp/residue.db.backup')
    request.addfinalizer(lambda: shutil.move('/tmp/residue.db.backup', '/tmp/residue.db'))
    return init_db
...
class TestNamingConventions(object):
    ...
    @pytest.mark.parametrize('sqltext,expected', [
        ...
        ('0123456789012345678901234567890123', '3562d89a2fbf525580778c8d000cb00a')
    ])
...
class TestCrudRead(object):
    ...
    def test_read_with_multiple_queries(self, db):
        self.assert_read_result(
            [db.turner_account, db.hooch_account], [query_from(db.turner_account), query_from(db.hooch_account)])
    ...
    def test_read_with_multiple_queries_and_one_data_spec(self, db):
        ...
        self.assert_read_result(
            expected, [query_from(db.turner_account), query_from(db.hooch_account)], {'username': True})
        self.assert_read_result(
            expected, [query_from(db.turner_account), query_from(db.hooch_account)], [{'username': True}])
    ...
    def test_read_with_two_models_and_two_data_specs(self, db):
        ...
        expected = self.extract(
            db.turner_account, '_model', 'id', 'username') + self.extract(db.pirate, '_model', 'id', 'user_id')
...
class TestCrudUpdate(object):
    def test_single_update(self):
        Session.crud.update(
            {'_model': 'Account', 'field': 'username', 'value': 'turner_account'}, {'password': 'changing'})
        with Session() as session:
    ...
    def test_handle_bad_relation_type(self, db):
        pytest.raises(
            CrudException, Session.crud.update, query_from(db.turner), {'employees': 'not a dict or sequence of dicts'})
    ...
    def test_create_foreign_relation_with_multiple_specs(self, db):
        Session.crud.update(
            [query_from(db.turner_account), query_from(db.hooch_account)], 2 * [{'user': {'name': 'New User'}}])
        with Session() as session:
            assert 3 == len(session.query(User).all())
            assert 'New User' == session.account('turner_account').user.name
            assert 'New User' == session.account('hooch_account').user.name
...
class TestCrudDelete(object):
    def test_delete_cascades_to_tags(self):
        pytest.skip('sqlite is not compiled with foreign key support on Jenkins; '
