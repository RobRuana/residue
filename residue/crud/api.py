...
import six
from pockets import classproperty, collect_superclass_attr_names, is_data, is_listy, mappify
...
def _extract_sort_field(model, value, index=0):
    ...
    if field and isinstance(field, six.string_types) and model:
        ...
        if not is_instrumented and not isinstance(attr, ClauseElement):
            raise ValueError(
                'SQLAlchemy model classes may only be sorted '
    ...
    if field:
        ...
        primary_keys = get_primary_key_column_names(model)
        index = max(index, len(primary_keys) - 1)
...
def normalize_sort(model, sort):
    ...
    if isinstance(sort, six.string_types):
        ...
        for i, s in enumerate(sort):
            sorters.extend(normalize_sort(model, s, index=i))
        return sorters
    elif isinstance(sort, dict):
        field = sort.get('property', sort.get('fields', sort.get('field', [])))
        direction = sort.get('direction', sort.get('dir', 'asc')).lower()
        return [{
            'field': _extract_sort_field(model, field),
            'dir': direction
        }]
    else:
        return [{'field': name, 'dir': 'asc'}
                for name in get_primary_key_column_names(model)]
...
def normalize_data(data, count=1):
    ...
    if isinstance(data, six.string_types):
        ...
        if any(isinstance(element, six.string_types) for element in data):
            # Singular list of strings, so wrap it and go from there
            data = [data]
        # Is this a list of strings? Who can say!
        ...
        if isinstance(q, six.string_types):
            ...
            if 'and' in q or 'or' in q:
                ...
                if not isinstance(q[op], (list, set, tuple)):
                    raise ValueError(
                        'Clause must be of type list, set, or tuple not '
                        '{}, given {}'.format(type(q[op]), q[op]))

                q[op] = normalize_query(
                    q[op], False, q.get('_model', supermodel))
                ...
                if len(q[op]) == 1:
                    ...
                    if model is None:
                        raise ValueError(
                            'Clause objects must have a "_model" attribute, '
                            'given:\n{}'.format(q))
            ...
            if '_model' in q:
                ...
                raise ValueError(
                    'Query objects must have a "_model" attribute, '
                    'given:\n{}'.format(q))
        else:
            raise ValueError(
                'Query objects must be either a dict or string, '
                'given {}:\n{}'.format(type(q), q))
    return queries
...
class CrudApi(object):
    ...
    def crud_subscribes(func):
        ...
        class subscriber(object):
            ...
            def subscribes(self):
                ...
                return CrudApi._get_models(message.get('params')) if message else []
        ...
        class notifier(object):
            def __call__(self, *args, **kwargs):
                try:
                    ...
                    models = CrudApi._get_models(args, kwargs)
        ...
        for filter in filters:
            ...
            if getattr(model, '_crud_perms', {}).get('read', True):
                if filter.get('groupby', False):
                    ...
                    rows = CrudApi._filter_query(session.query(func.count(columns[0]), *columns), model, filter).all()
                    result['count'] = []
                    for row in rows:
                        count = {'count': row[0]}
                        index = 1
    ...
    @crud_subscribes.__func__
    def read(session, query, data=None, order=None, limit=None, offset=0):
        """
    ...
    def delete(session, query):
        ...
        """
        deleted = 0
        for filter in normalize_query(query):
            model = Session.resolve_model(filter['_model'])
            crud_perms = getattr(model, '_crud_perms', {})
            if crud_perms.get('can_delete', False):
